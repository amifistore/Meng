...
async def produk_pilih_callback(update, context):  # ✅ TAMBAHKAN ASYNC
    ...
    if data.startswith("produk_static|"):
        ...
        if idx < 0 or idx >= len(produk_list):
            await query.edit_message_text("❌ Produk tidak valid.", reply_markup=reply_main_menu(user.id))  # ✅ TAMBAHKAN AWAIT
        ...
        await query.edit_message_text("Kembali ke menu utama.", reply_markup=reply_main_menu(user.id))  # ✅ TAMBAHKAN AWAIT
        return ConversationHandler.END

    else:
        await query.edit_message_text("❌ Callback tidak dikenali.", reply_markup=reply_main_menu(user.id))  # ✅ TAMBAHKAN AWAIT
        return ConversationHandler.END
