import os, json, uuid, base64, logging, sqlite3, time, threading, random
...
def update_produk_cache_background():
    ...
    try:
        ...
        res = requests.get("https://panel.khfy-store.com/api_v3/cek_stock_akrab", timeout=10)
        data = res.json()
...
def konfirmasi_step(update: Update, context: CallbackContext):
    ...
    # Gunakan harga yang sudah disimpan di user_data
    harga = produk.get("harga", 0)
    if harga <= 0:
        # Fallback jika harga tidak ada
    ...
    try:
        ...
        update.message.reply_text(f"❌ Gagal request: {e}", reply_markup=get_menu(user.id))
        return ConversationHandler.END
        
    if not data or not data.get("refid"):
        update.message.reply_text("❌ Gagal membuat transaksi.", reply_markup=get_menu(user.id))
        return ConversationHandler.END
        
    status_text = data.get('status', '-')
    emoji = "✅" if "sukses" in status_text.lower() else ("❌" if "gagal" in status_text.lower() else "⏳")
    saldo_now = get_saldo(user.id)
    if "sukses" in status_text.lower():
        kurang_saldo(user.id, harga)
        saldo_now -= harga
        
    log_riwayat(reffid, user.id, produk["kode"], tujuan, harga, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), status_text, data.get("message", ""))
    update.message.reply_text(
        f"{emoji} <b>TRANSAKSI BERHASIL</b>\n\n"
        f"📦 <b>Produk:</b> [{produk['kode']}] {produk['nama']}\n"
        f"📱 <b>Tujuan:</b> {tujuan}\n"
        f"🔖 <b>RefID:</b> <code>{reffid}</code>\n"
        f"📊 <b>Status:</b> {status_text}\n"
        f"💰 <b>Saldo Sekarang:</b> Rp {saldo_now:,}",
        parse_mode=ParseMode.HTML,
        reply_markup=get_menu(user.id)
    )
    return ConversationHandler.END
...
def topup_amount_step(update, context):
    ...
    user = update.effective_user
    sukses, hasil = generate_qris(nominal, QRIS_STATIS)
    if sukses:
        try:
            img_bytes = base64.b64decode(hasil)
            topup_id = str(uuid.uuid4())
            insert_topup_pending(topup_id, user.id, user.username or "", user.full_name, nominal, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "pending")
...
def callback_router(update: Update, context: CallbackContext):
    ...
    if data == "main_menu":
        ...
        admin_topup_pending_menu(update, context)
        return ConversationHandler.END
...
def main():
    ...
    # Pre-load cache produk saat startup
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            INPUT_KODE_UNIK: [MessageHandler(Filters.text & ~Filters.command, admin_generate_kode_step)],
        },
        fallbacks=[
            CommandHandler('start', start),
